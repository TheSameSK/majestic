...
namespace voiden_menu {
    ...
    std::vector< menu_column > build_layout(int tab) {
        ...
        if (tab == tab_aimbot) {
            ...
            menu_block tracers;
            tracers.id = "tracers";
            tracers.title = "TRACERS";
            tracers.icon = eye_line;
            tracers.rows.push_back(make_toggle("Enabled", "visual", "tracers", false));
            tracers.rows.push_back(dim_when_off(make_color("Color", "visual", "tracers_r", "tracers_g", "tracers_b", "tracers_a"), "tracers"));
