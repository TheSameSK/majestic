...
namespace executor_payload {
inline std::string source() {
    return std::string(
R"NCTPAYLOAD(// payload guard
(function () {
...
function sendBridgeHello() {
  try {
    if (socket && socket.readyState === 1) {
      socket.send(JSON.stringify({ type: 'BRIDGE_HELLO', session: globalThis.noctuaBridgeSession || '', pr)NCTPAYLOAD"
R"NCTPAYLOAD(oof: globalThis.noctuaBridgeProof || '' }));
    }
  } catch (e) {}
}
...
R"NCTPAYLOAD(  try { callback(state.value); } catch (e) {}
        }
      },
    });
    return handle;
...
function connect() {
  ...
  clearReconnect();
  stopSocket();
  socket = new alt.WebSocketClient()NCTPAYLOAD"
R"NCTPAYLOAD(WS_URL);
